...
先读 README，Windows 另读 WINDOWS.md。检查 Python 3.9+、Node.js 20+，复用已有环境；缺少依赖时说明所需步骤。

在独立目录获取 main 分支源码，不覆盖已有账户或其他项目。
Mac：bash install.sh --source "$PWD"
Windows：npm ci --ignore-scripts，然后运行 start-windows.cmd。
已有 Mac 安装不会自动升级，请按 MAC.md 核对运行副本。

验证本地配置页打开后，让我选择数据源并在官方页面授权。继续引导我确认额度、启用自己的 iCloud、在 iPhone Scriptable 运行生成脚本并添加组件。电脑写入不等于手机收到，需核对采集时间。
